...
class M_tran_lain extends CI_Model {
    ...
    function simpan_transaksi_lain($datanya)
    {
        return $this->db->insert('transaksi_lain',$datanya);
    }
    ...
    function get_id_item_lain($id)
    {
        ...
        $this->db->where('id_item_transaksi_lain',$id);
        $query = $this->db->get('item_transaksi_lain');
        //return $query->result_array();
        return $query;
    }
    ...
    {
        return $this->db->insert('item_transaksi_lain',$data);
    }
    ...
    function count_transaksi($like, $id) 
    {
        $like != '' ? $this->db->like($like) : '';
        $this->db->where('id_transaksi',$id);
        return $this->db->count_all_results('v_item_transaksi_lain');
    }
    ...
    function get_transaksi($like, $sidx, $sord, $limit, $start, $kun) 
    {
        ...
        $this->db->where('id_transaksi', $kun);
        $this->db->order_by($sidx, $sord);
        return $this->db->get('v_item_transaksi_lain', $limit, $start);
    }
    
    function delete_item_transaksi($id) 
    {
	$this->db->where('id_item_transaksi_lain', $id);
	$this->db->delete('item_transaksi_lain'); 
    }
