...
class M_tran_penunjang extends CI_Model {
    ...
    function simpan_transaksi_penunjang($datanya)
    {
        return $this->db->insert('transaksi_penunjang',$datanya);
    }
    ...
    */    
    ...
    {
        return $this->db->insert('item_transaksi_penunjang',$data);
    }
 ...
 */
    ...
    function count_transaksi($like, $id) 
    {
        $like != '' ? $this->db->like($like) : '';
        $this->db->where('id_transaksi',$id);
        return $this->db->count_all_results('v_item_transaksi_penunjang');
    }    
    function get_transaksi($like, $sidx, $sord, $limit, $start, $kun) 
    {
        ...
        $this->db->where('id_transaksi', $kun);
        $this->db->order_by($sidx, $sord);
        return $this->db->get('v_item_transaksi_penunjang', $limit, $start);
    }
    /*
    ...
    {
	$this->db->where('id_item_transaksi_penunjang', $id);
	$this->db->delete('item_transaksi_penunjang'); 
    }
    ...
    function cariproviderpenunjang($keyword)
    {
        $this->db->select('id_provider, nama_provider');
        $this->db->like('nama_provider',$keyword,'after');
        $this->db->where('idjenis_provider','6');
        $this->db->limit('10');
        $query = $this->db->get('master_provider');
        return $query->result_array();
    }
