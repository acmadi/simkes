...
class Gigi extends CI_Controller {
    ...
    {
        parent::__construct();
        $this->load->model(array('m_tran_gigi'));
	$this->load->helper('url'); // Load Helper URL CI
    }
    
    function index() 
    {
	$this->load->view('transaksi/gigi'); //Default di arahkan ke function grid
    }
    ...
    function lookpegawai()
    {
        ...
        $data['response'] = 'false';
        $query = $this->m_tran_gigi->caripegawai($keyword);
        if( ! empty($query) )
        {
            $data['response'] = 'true';
            $data['message'] = array();
    ...
    function lookdokter()
    {
        $keyword = $this->input->post('term');
        $data['response'] = 'false';
        $query = $this->m_tran_gigi->caridokter($keyword);
        if( ! empty($query) )
        {
            $data['response'] = 'true';
            $data['message'] = array();
    ...
    function looklabgigi()
    {
        $keyword = $this->input->post('term');
        $data['response'] = 'false';
        $query = $this->m_tran_gigi->carilabgigi($keyword);
        if( ! empty($query) )
        {
            $data['response'] = 'true';
            $data['message'] = array();
    ...
    function lookdiagnosa()
    {
        $keyword = $this->input->post('term');
        $data['response'] = 'false';
        $query = $this->m_tran_gigi->caridiagnosa($keyword);
        if( ! empty($query) )
        {
            $data['response'] = 'true';
            $data['message'] = array();
    ...
    function lookdosis()
    {
        $keyword = $this->input->post('term');
        $data['response'] = 'false';
        $query = $this->m_tran_gigi->caridosis($keyword);
        if( ! empty($query) )
        {
            $data['response'] = 'true';
            $data['message'] = array();
    ...
    function looktagihan()
    {
        ...
        $data['response'] = 'false';
        $query = $this->m_tran_gigi->caritagihan($keyword, $id);
        if( ! empty($query) )
        {
            $data['response'] = 'true';
            $data['message'] = array();
    ...
    function lookrekomendasi()
    {
        $keyword = $this->input->post('term');
        $data['response'] = 'false';
        $query = $this->m_tran_gigi->carirekomendasi($keyword);
        if( ! empty($query) )
        {
            $data['response'] = 'true';
            $data['message'] = array();
    ...
    function bagian()
    {
        //$id = "1";
        $id = $this->input->post('id');
        $query = $this->m_tran_gigi->get_bagian($id);
        if( ! empty($query) )
        {
            foreach( $query as $row )
            {
    ...
    function rujukan()
    {
        $query = $this->m_tran_gigi->get_rujukan();
        ...
        if( ! empty($query) )
        {
            ...
            {
                echo "<option value=";
                echo $row['idjenis_provider'];
                echo ">";
                echo $row['jenis_provider'];
                echo "</option>";
            }
        }
    }
    ...
    function kunjungan()
    {
        $query = $this->m_tran_gigi->get_kunjungan();
    ...
    function tagihan()
    {
        $query = $this->m_tran_gigi->get_tagihan();
    ...
    {
        $id = $this->input->post('id');
        $query = $this->m_tran_gigi->get_pasien($id);
    ...
    function simpandata()
    {
        ...
        {            
            $query = $this->m_tran_gigi->get_id_dokter();
            if( ! empty($query) )
            {
                foreach( $query as $row )
                {
                ...
                $this->m_tran_gigi->simpan_dokter($data);
                $id_dokter=$id_dok;
            }            
        }        
        
        $lab_gigi = $this->input->get('lab_gigi');
        $id_lab_gigi = $this->input->get('id_lab_gigi');
        if ($id_lab_gigi=='undefined')
        {            
            $query = $this->m_tran_gigi->get_id_labgigi();
            if( ! empty($query) )
            {
                foreach( $query as $row )
                {
                    $id_lab = $row['id_provider']+1;
                }
                $data=array('id_provider'=>$id_lab, 'nama_provider'=>$lab_gigi, 'idjenis_provider'=>'2');
                $this->m_tran_gigi->simpan_labgigi($data);
                $id_lab_gigi=$id_lab;
            }            
        }
        ...
        $query = $this->m_tran_gigi->get_id_transaksi();
        $row = $query->row();
        ...
        $query3=$this->m_tran_gigi->simpan_transaksi($data_tran);
        if ($query3){
            $data_tran2=array('id_transaksi'=>$id_transaksi,'no_surat'=>$no_surat,'no_bukti'=>$no_bukti,'restitusi'=>$restitusi,'id_dokter'=>$id_dokter,'id_provider'=>$id_lab_gigi);
            $query4=$this->m_tran_gigi->simpan_transaksi_gigi($data_tran2);
            
            $data_tran3=array('id_transaksi'=>$id_transaksi,'buku_besar'=>$buku_besar);
            $query5=$this->m_tran_gigi->simpan_transaksi_buku($data_tran3);
    ...
    function simpandiagnosa()
    {
        ...
        {
            $query = $this->m_tran_gigi->get_id_diagnosa();
            if( ! empty($query) )
            {
                $row = $query->row();
                $id=$row->id_diagnosa+1;    
                $data = array('id_diagnosa'=>$id,'nama_diagnosa'=>$diagnosa);
                 $this->m_tran_gigi->simpan_diagnosa($data);
                $id_diagnosa=$id;
            }
        } else {
            $id_diagnosa=$id_diag;
    ...
    function simpanitem()
    {
        ...
        $rekomendasi;
        if ($rekom=='undefined') {           
            $query = $this->m_tran_gigi->get_id_rekomendasi();
            if( ! empty($query) )
            {
                $row = $query->row();
                $id = $row->id_rekomendasi+1;
                $data = array('id_rekomendasi'=>$id,'nama_rekomendasi'=>$nm_rekom);
                $this->m_tran_gigi->simpan_rekomendasi($data);                
                $rekomendasi = $id;
            }
        } else {
            $rekomendasi=$rekom;
        ...
        $datanya=array('id_transaksi'=>$kunjungan,'disetujui'=>$disetujui,'hrg_satuan'=>$harga_satuan,'jumlah'=>$jumlah,'satuan'=>$satuan,
                        'id_item'=>$id_item,'id_rekomendasi'=>$rekomendasi);
        $query3=$this->m_tran_gigi->simpan_item_transaksi($datanya);
        if ($query3){
            echo "sukses";           
        }else{
            echo "gagal";
	...
	$count = $this->m_tran_gigi->count_diagnosa2($where, $kun);
	...
	$data1 = $this->m_tran_gigi->get_diagnosa2($where, $sidx, $sord, $limit, $start, $kun)->result();
	$responce->page = $page;
...
function json2() 
	...
	$count = $this->m_tran_gigi->count_transaksi($where, $kun);
	...
	$data1 = $this->m_tran_gigi->get_transaksi($where, $sidx, $sord, $limit, $start, $kun)->result();
	$responce->page = $page;
	$responce->total = $total_pages;
	$responce->records = $count;
	...
	foreach($data1 as $line)
            {
                ...
                $responce->rows[$i]['id']   = $line->id_item_transaksi_gigi;
                $responce->rows[$i]['cell'] = array($line->id_item_transaksi_gigi,
                    '',
                    $line->jenis_item,
                    $line->nama_item,
                    $line->harga_item,
