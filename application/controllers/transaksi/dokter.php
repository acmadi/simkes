...
class Dokter extends CI_Controller {
    ...
    {
        parent::__construct();
        $this->load->model('m_tran_dokter');
	$this->load->helper('url'); // Load Helper URL CI
    }
    
    function index() 
    {
	$this->load->view('transaksi/dokter'); //Default di arahkan ke function grid
    }
    ...
    function lookdokter()
    {
        $keyword = $this->input->post('term');
        $data['response'] = 'false';
        $query = $this->m_tran_dokter->caridokter($keyword);
        if( ! empty($query) )
        {
            $data['response'] = 'true';
            $data['message'] = array();
            foreach( $query as $row )
            {
                $data['message'][] = array(
                                        ...
                                        'satuan'=>$row['tarif_dokter'],
                                        'standar'=>$row['tarif_standar'],
                                     );
            }
        }
        echo json_encode($data);
    ...
    function lookapotek()
    {
        $keyword = $this->input->post('term');
        $data['response'] = 'false';
        $query = $this->m_tran_dokter->cariapotek($keyword);
        if( ! empty($query) )
        {
            $data['response'] = 'true';
            $data['message'] = array();
    ...
    function lookdiagnosa()
    {
        $keyword = $this->input->post('term');
        $data['response'] = 'false';
        $query = $this->m_tran_dokter->caridiagnosa($keyword);
        if( ! empty($query) )
        {
            $data['response'] = 'true';
            $data['message'] = array();
    ...
    function lookdosis()
    {
        $keyword = $this->input->post('term');
        $data['response'] = 'false';
        $query = $this->m_tran_dokter->caridosis($keyword);
        if( ! empty($query) )
        {
            $data['response'] = 'true';
            $data['message'] = array();
    ...
    function looktagihan()
    {
        ...
        $data['response'] = 'false';
        $query = $this->m_tran_dokter->caritagihan($keyword, $id);
        if( ! empty($query) )
        {
            $data['response'] = 'true';
            $data['message'] = array();
    ...
    function lookrekomendasi()
    {
        $keyword = $this->input->post('term');
        $data['response'] = 'false';
        $query = $this->m_tran_dokter->carirekomendasi($keyword);
        if( ! empty($query) )
        {
            $data['response'] = 'true';
            $data['message'] = array();
    ...
    function bagian()
    {
        //$id = "1";
        $id = $this->input->post('id');
        $query = $this->m_tran_dokter->get_bagian($id);
        if( ! empty($query) )
        {
            foreach( $query as $row )
            {
    ...
    function tagihan()
    {
        $query = $this->m_tran_dokter->get_tagihan();
    ...
    {
        $id = $this->input->post('id');
        $query = $this->m_tran_dokter->get_pasien($id);
    ...
    function simpandata()
    {
        ...
        {            
            $query = $this->m_tran_dokter->get_id_dokter();
            if( ! empty($query) )
            {
                foreach( $query as $row )
                {
                ...
                $this->m_tran_dokter->simpan_dokter($data);
                $id_dokter=$id_dok;
            }            
        }        
        $tarif_standart = $this->input->get('tarif_standart');
        $tarif_satuan = $this->input->get('tarif_satuan');
        ...
        $query = $this->m_tran_dokter->get_id_transaksi();
        $row = $query->row();
        ...
        $query3=$this->m_tran_dokter->simpan_transaksi($data_tran);
        if ($query3){
            $data_tran2=array('id_transaksi'=>$id_transaksi,'no_surat'=>$no_surat,'no_bukti'=>$no_bukti,'restitusi'=>$restitusi,'id_dokter'=>$id_dokter,'tarif_satuan_dokter'=>$tarif_satuan,'tarif_standar_dokter'=>$tarif_standart);
            $query4=$this->m_tran_dokter->simpan_transaksi_dokter($data_tran2);
            
            $data_tran3=array('id_transaksi'=>$id_transaksi,'buku_besar'=>$buku_besar);
            $query5=$this->m_tran_dokter->simpan_transaksi_buku($data_tran3);
    ...
    function simpandiagnosa()
    {
        ...
        {
            $query = $this->m_tran_dokter->get_id_diagnosa();
            if( ! empty($query) )
            {
                $row = $query->row();
    ...
    function simpanitem()
    {
        ...
        if ($id_item=='undefined') {           
            $query = $this->m_tran_dokter->get_id_item();
            if( ! empty($query) )
            {
                $row = $query->row();
                $idi=$row->id_item+1;
        ...
        if ($dos=='undefined') {           
            ...
            if( ! empty($query) )
            {
                ...
                $this->m_tran_dokter->simpan_dosis($data);                
                $dosis=$id;
            }
        } else {
            $dosis=$dos;
        ...
        $rekomendasi;
        if ($rekom=='undefined') {           
            $query = $this->m_tran_dokter->get_id_rekomendasi();
            if( ! empty($query) )
            {
                $row = $query->row();
                $id = $row->id_rekomendasi+1;
                $data = array('id_rekomendasi'=>$id,'nama_rekomendasi'=>$nm_rekom);
                $this->m_tran_dokter->simpan_rekomendasi($data);                
                $rekomendasi = $id;
            }
        } else
        {
        ...
        $query4 = $this->m_tran_dokter->simpan_item_dosis($data);
        if ($query4){
            $datanya=array('id_transaksi'=>$kunjungan,'disetujui'=>$disetujui,'hrg_satuan'=>$harga_satuan,'jumlah'=>$jumlah,
                        'id_item'=>$id_item,'id_rekomendasi'=>$rekomendasi);
            $query3=$this->m_tran_dokter->simpan_item_transaksi($datanya);
            if ($query3){
                echo "sukses";
            }else{
                echo "gagal";
	...
	$count = $this->m_tran_dokter->count_diagnosa2($where, $kun);
	...
	$data1 = $this->m_tran_dokter->get_diagnosa2($where, $sidx, $sord, $limit, $start, $kun)->result();
	$responce->page = $page;
	...
	$id=$this->input->post('id');
        ...
        $query = $this->m_tran_dokter->get_id_item_lain($id);
        $row = $query->row();
        $id_item = $row->id_item;
        ...
        switch ($oper) 
	{
            case 'del':
                ...
                $this->m_tran_dokter->delete_dosis($id_item,$id_trans);
            break;
	}
    }
...
function json2() 
	...
	$count = $this->m_tran_dokter->count_transaksi($where, $kun);
	...
	$data1 = $this->m_tran_dokter->get_transaksi($where, $sidx, $sord, $limit, $start, $kun)->result();
	$responce->page = $page;
	$responce->total = $total_pages;
	$responce->records = $count;
	...
	foreach($data1 as $line)
            {
                ...
                $responce->rows[$i]['id']   = $line->id_transaksi_dokter;
                $responce->rows[$i]['cell'] = array($line->id_transaksi_dokter,
                    '',
                    $line->jenis_item,
                    $line->nama_item,
                    $line->harga_item,
