...
class Optik extends CI_Controller
{
	...
	function __construct() {
		...
		$this->load->model('m_optik'); // Load Model m_jqgrid
	...
	function grid() 
	{
		$this->load->view('master/optik'); // Load View jqgrid
	}
	function json() 
	{
		...
		$count = $this->m_optik->count_optik($where);
		...
		$data1 = $this->m_optik->get_optik($where, $sidx, $sord, $limit, $start)->result();
	...
	function crud() 
	{
		...
		$optik_nama=$this->input->post('nama_provider');
		$optik_alamat=$this->input->post('almt_provider');
		$optik_langg=$this->input->post('langg_provider');
		$optik_email=$this->input->post('email_provider');
		$optik_fax=$this->input->post('fax_provider');
		$optik_telp=$this->input->post('tlp_provider');
		...
		{
	        case 'add':
				$datanya=array('nama_provider'=>$optik_nama,'almt_provider'=>$optik_alamat,'langg_provider'=>$optik_langg,'email_provider'=>$optik_email,'fax_provider'=>$optik_fax,'tlp_provider'=>$optik_telp,'idjenis_provider'=>4);
				$this->m_optik->insert_optik($datanya);
				break;
	        case 'edit':
	            $datanya=array('nama_provider'=>$optik_nama,'almt_provider'=>$optik_alamat,'langg_provider'=>$optik_langg,'email_provider'=>$optik_email,'fax_provider'=>$optik_fax,'tlp_provider'=>$optik_telp,'idjenis_provider'=>4);
				$this->m_optik->update_optik($id_provider, $datanya);
	            break;
	        case 'del':
	            $this->m_optik->delete_optik($id_provider);
	        break;
		}
	}
	...
	else
	{
			...
			else
			{
                            ...
                            $dataexcel[$i-1]['idjenis_provider'] = "4";
                            }
                            }
                            $check=$data[2]['A'];
                            $data['check']=$check;
                            ...
                            for($i=1;$i<=count($dataexcel);$i++)
                            {
                             try
                             {
                                     $this->m_optik->insert_optik($dataexcel[$i]);
	...
	$tulis=$this->excel->setActiveSheetIndex(0);
        ...
        foreach($data1 as $line)
		{
      	...
      	// Rename worksheet
	$this->excel->getActiveSheet()->setTitle('Optik');
	...
	header('Content-Disposition: attachment;filename="Master Optik.xls"');
