...
class Lab extends CI_Controller {
	...
	function __construct() {
		...
		$this->load->model('m_lab'); // Load Model m_jqgrid
	...
	function grid() 
	{
		$this->load->view('master/lab'); // Load View jqgrid
	}
	function json() 
	{
		...
		$count = $this->m_lab->count_lab($where);
		...
		$data1 = $this->m_lab->get_lab($where, $sidx, $sord, $limit, $start)->result();
		...
		foreach($data1 as $line)
		{
			...
			$responce->rows[$i]['cell'] = array($line->id_provider,$line->nama_provider,$line->almt_provider,$line->langg_provider,$line->email_provider,$line->fax_provider,$line->tlp_provider);
			$i++;
		}
	...
	function crud() 
	{
		...
		$lab_nama=$this->input->post('nama_provider');
		$lab_alamat=$this->input->post('almt_provider');
		$lab_langg=$this->input->post('langg_provider');
		$lab_email=$this->input->post('email_provider');
		$lab_fax=$this->input->post('fax_provider');
		$lab_telp=$this->input->post('tlp_provider');
		...
		{
	        case 'add':
				$datanya=array('nama_provider'=>$lab_nama,'almt_provider'=>$lab_alamat,'langg_provider'=>$lab_langg,'email_provider'=>$lab_email,'fax_provider'=>$lab_fax,'tlp_provider'=>$lab_telp,'idjenis_provider'=>3);
				$this->m_lab->insert_lab($datanya);
				break;
	        case 'edit':
	            $datanya=array('nama_provider'=>$lab_nama,'almt_provider'=>$lab_alamat,'langg_provider'=>$lab_langg,'email_provider'=>$lab_email,'fax_provider'=>$lab_fax,'tlp_provider'=>$lab_telp,'idjenis_provider'=>3);
				$this->m_lab->update_lab($id_provider, $datanya);
	            break;
	        case 'del':
	            $this->m_lab->delete_lab($id_provider);
	        break;
		}
	}
	...
	else
	{
			...
			else
			{
                            ...
                            $dataexcel[$i-1]['idjenis_provider'] = "3";
                            }
                            }
                            $check=$data[2]['A'];
                            $data['check']=$check;
                            ...
                            for($i=1;$i<=count($dataexcel);$i++)
                            {
                             try
                             {
                                     $this->m_lab->insert_lab($dataexcel[$i]);
	...
	$tulis=$this->excel->setActiveSheetIndex(0);
        ...
        foreach($data1 as $line)
		{
      	...
      	// Rename worksheet
	$this->excel->getActiveSheet()->setTitle('Lab');
	...
	header('Content-Disposition: attachment;filename="Master Laboratorium.xls"');
