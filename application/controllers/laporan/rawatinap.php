...
class Rawatinap extends CI_Controller {
   // $kun="";
        ...
        function index()
    {
        ...
        $this->load->view('laporan/rawatinap'); //Default di arahkan ke function grid
    }
	...
	function json() 
	{
		...
		//$searchField='nama_diagnosa';
                ...
                if($jenis == 'd')
                {
                $count = $this->m_laporan->count_RawatinapDokter($searchField,$searchString,$rayon,$wilayah,$mitra,$bulan,$tahun,$filter,'');
		}
                else
                {
                $count = $this->m_laporan->count_Rawatinap($searchField,$searchString,$rayon,$wilayah,$mitra,$bulan,$tahun,$filter,'');
		}
		...
		if($jenis == 'd')
                {
                $data1 = $this->m_laporan->get_RawatinapDokter($searchField,$searchString, $sidx, $sord, $limit, $start,$rayon,$wilayah,$mitra,$bulan,$tahun,$filter,'')->result();
                }
                else
                {
