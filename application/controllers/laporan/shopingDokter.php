...
class ShopingDokter extends CI_Controller {
    ...
    function index()
    {
	$this->load->view('laporan/shopingDokter'); //Default di arahkan ke function grid
	...
	/*if ($_GET['_search'] == 'true')
	{
		...
		# End #
		//$searchField='nama_shopingDokter';
                //$searchField='';
                //$searchString=$shopingDokter;
                //$searchString='';
	...
	foreach($data1 as $line)
	{
			...
			$detail="<a href='$line->id_transaksi' class='detail_shoping'><span class='ui-icon ui-icon-document'></span></a>";
            $responce->rows[$i]['cell'] = array($line->id_transaksi,$line->tgl_transaksi,$line->tgl_kunjungan,
                                ...
                                $line->nama_dokter,$line->jumlah_hari,$line->biaya,$detail);
            $i++;
	}
	...
	$tulis=$this->excel->setActiveSheetIndex(0);
        ...
        $tulis->setCellValue('J1', "Kategori");
        $tulis->setCellValue('K1', "Biaya");
        ...
        foreach($data1 as $line)
	{
            ...
            $tulis->setCellValue('J'.$i, $line->kategori);
            $tulis->setCellValue('K'.$i, $line->biaya);
