...
class Rawatjalan2 extends CI_Controller {
   // $kun="";
	...
	$tahun=$this->m_laporan->gettahunrawatjalan()->result_array();
        ...
        $data['tahun']=$tahun;
        $data['item']=$jenis;
        $this->load->view('laporan/rawatjalan2',$data); //Default di arahkan ke function grid
    }
	...
	function json() 
	{
		...
		$count = $this->m_laporan->count_Rawatjalan2($searchField,$searchString,$rayon,$wilayah,$mitra,$bulan,$tahun,$filter,$jenis);
		...
		$data1 = $this->m_laporan->get_Rawatjalan2($searchField,$searchString, $sidx, $sord, $limit, $start,$rayon,$wilayah,$mitra,$bulan,$tahun,$filter,$jenis)->result();
		...
		foreach($data1 as $line)
		{
			...
			$responce->rows[$i]['cell'] = array($line->id_transaksi,$line->tgl_kunjungan,$line->nip,$line->nama_karyawan,$line->ap,$line->nama_tertanggung,$line->status,$line->nama_provider,$line->buku_besar,$line->nama_diagnosa,$line->nama_item,$line->jumlah,$line->total);
			$i++;
		}
	...
	// Add some data
		...
		$data1 = $this->m_laporan->get_Rawatjalan2($searchField,$searchString, $sidx, $sord, $limit, $start,$rayon,$wilayah,$mitra,$bulan,$tahun,$filter,$jenis)->result();
                $tulis=$this->excel->setActiveSheetIndex(0);
                        ...
                        $tulis->setCellValue('J1', "Buku Besar");
                        $tulis->setCellValue('K1', "Diagnosa");
                        $tulis->setCellValue('L1', "Item");
                        $tulis->setCellValue('M1', "Jumlah");
        ...
        foreach($data1 as $line)
		{
			$tulis->setCellValue('A'.$i, $i-1);
                        ...
                        $tulis->setCellValue('J'.$i, $line->buku_besar);
                        $tulis->setCellValue('K'.$i, $line->nama_diagnosa);
                        $tulis->setCellValue('L'.$i, $line->nama_item);
                        $tulis->setCellValue('M'.$i, $line->jumlah);
