...
class Kunjungankesehatan extends CI_Controller {
   // $kun="";
    ...
    function __construct()
    {
        ...
        function index()
    {
	$tahun=$this->m_laporan->gettahunkunjungankesehatan()->result_array();
        //print_r($tahun);

        $data['tahun']=$tahun;
        $this->load->view('laporan/kunjungankesehatan',$data); //Default di arahkan ke function grid
    }
	...
	function json() 
	{
		...
		$count = $this->m_laporan->count_Kunjungankesehatan($searchField,$searchString,$rayon,$wilayah,$mitra,$bulan,$tahun,$filter,$jenis);
		...
		$data1 = $this->m_laporan->get_Kunjungankesehatan($searchField,$searchString, $sidx, $sord, $limit, $start,$rayon,$wilayah,$mitra,$bulan,$tahun,$filter,$jenis)->result();
	...
	// Add some data
        ...
        foreach($data1 as $line)
		{
      	...
      	// Rename worksheet
	$this->excel->getActiveSheet()->setTitle('Laporan Kunjungan Kesehatan');
	...
	header('Content-Disposition: attachment;filename="Laporan Kunjungan Kesehatan.xls"');
