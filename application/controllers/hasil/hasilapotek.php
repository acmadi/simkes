...
class  Hasilapotek extends CI_Controller {
   // $kun="";
    ...
    {
        parent::__construct();
        $this->load->model(array('m_rekomendasi','m_hasil_transaksi','m_transaksi','m_karyawan','m_tran_apotek','m_tertanggung','m_item','m_dokter','m_apotek','m_view_hasilapotek'));
        ...
        function index()
        {
	$this->load->view('hasil/hasilapotek'); //Default di arahkan ke function grid
	...
	function json() 
	{
		...
		$count = $this->m_hasil_transaksi->count_Apotek($searchField,$searchString,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2,$jns_tanggal);
		...
		$data1 = $this->m_hasil_transaksi->get_Apotek($searchField,$searchString, $sidx, $sord, $limit, $start,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2,$jns_tanggal)->result();
		...
		foreach($data1 as $line)
		{
                ...
                else
                {
                    ...
                    $responce->rows[$i]['cell'] = array($line->id_transaksi,$line->tgl_transaksi,$line->tgl_kunjungan,$line->no_surat,$line->no_bukti,$line->buku_besar,$line->restitusi,$line->nip,$line->nama_karyawan,$line->nama_tertanggung,$line->nama_rujukan,$line->nama_dokter,$line->nama_provider,$line->no_dak,$diagnosa,$line->total,$line->koreksi,$line->selisih);
	...
	// Add some data
                ...
                foreach($data1 as $line)
		{
			$tulis->setCellValue('A'.$i, $i-1);
                        ...
                        $tulis->setCellValue('G'.$i, $line->buku_besar);
                        ...
                        $tulis->setCellValue('O'.$i, $line->no_dak);
                        $tulis->setCellValue('P'.$i, $line->nama_diagnosa);
	...
	$this->excel->setActiveSheetIndex(0);
        ...
        /*$this->tesekspor('Bismillah', 1);

        $this->excel->getSheet(0)->setTitle('Revenue');
        $this->excel->createSheet(2);
        $this->excel->setActiveSheetIndex(2);
        $this->excel->getActiveSheet()->setTitle('Spoon');
	...
	//exit;
        ...
        function tesekspor($title,$index)
        {
        $this->excel->createSheet($index);
        $tulis=$this->excel->setActiveSheetIndex($index);
        ...
        function do_impor()
	{
        ...
        $error = "";
	$pesan="";
	$fileElementName = 'uploadhasilapotek';
	...
	else
	{
			...
			else
			{
                            ...
                            for ($i = 7; $i <=count($sheetData); $i++)
                            {
                                ...
                                $status = strtolower($sheetData[$i]['G']);
                                ...
                                $jns_penyakit=$sheetData[$i]['AD'];
                                $rekomendasi=$sheetData[$i]['AA'];

//                                $tes =strtoupper($sheetData[$i]['G']);
//                                        if($tes != "SUAMI" )
//                                        {
//                                        $error .="DATA STATUS HANYA BOLEH SUAMI";
////                                        break;
//                                        }
//                                        else
//                                        {
...
//                                }
				          ...
				          $this->m_karyawan->insert_karyawan($datanya);
                                      ...
                                      if($id_karyawan==null or $penanggung == null)
                                      {
                                      ...
                                      if($pasien!=null)
                                      {
                                      $data_pasien = $this->m_tran_apotek->cek_pasien($id_karyawan,$pasien);
				          ...
				          $this->m_tertanggung->insert_tertanggung($datapasien);
                                      ...
                                      if($id_pasien==null)
                                      {
                                        ...
                                        $tran = $this->m_tran_apotek->simpan_transaksi($datatransaksi);
                                        if($tran)
                                        {
                                        ...
                                        if($rujukan!=null)
                                        {
                                        $datarujukan = $this->m_tran_apotek->carirujukan($rujukan);
                                        if($datarujukan)
                                        {
                                        ...
                                        {
                                        $pesan .="Cari Dokter";
                                        $datadokter = $this->m_tran_apotek->caridokterrujukan($nama_dokter,$id_rujukan,3);
                                        if($datadokter)
                                        {
				        ...
				        $doktere=$this->m_dokter->insert_dokter($datanya);
                                        ...
                                        $dataprovider = $this->m_tran_apotek->cariapotek($nama_provider);
                                        if($dataprovider)
                                        {
				        ...
				        $this->m_apotek->insert_apotek($dataapotek);
                                        ...
                                        $datatransaksiapotek =  array('id_transaksi'=>$id_transaksi,'id_rujukan'=>$id_rujukan,'id_dokter'=>$id_dokter,'id_provider'=>$id_provider,'no_bukti'=>$no_bukti,'no_surat'=>$no_surat,'restitusi'=>$restitusi);
                                        $query3 = $this->m_tran_apotek->simpan_data_transaksi($datatransaksiapotek);
                                    ...
                                    //$pesan .="Cari sheetdata[".$i."][o]item = ".$nama_item;
                                        ...
                                        $cari_item=$this->m_tran_apotek->caritagihan($nama_item,1);
                                    if( ! empty($cari_item))
                                    {
                                    ...
                                    if($dosis!=null)
                                    {
                                        $caridosis = $this->m_tran_apotek->caridosis($dosis);
                                        if($caridosis)
                                        {
                                    ...
                                    //$pesan .="Id Rekomendasi".$id_rekomendasi;
                                    $insertitem=$this->m_tran_apotek->simpan_item_transaksi($dataitemtransaksi);
                                    }
                                ...
                                {
                                        $caridiagnosa=$this->m_tran_apotek->caridiagnosa($diagnosa);
                                        if($caridiagnosa)
                                        {
                                        ...
                                        $this->m_tran_apotek->simpan_diagnosa($data_diagnosa);
                                        $caridiagnosa=$this->m_tran_apotek->caridiagnosa($diagnosa);
                                        $id_diagnosa = $caridiagnosa[0]['id_diagnosa'];
                                        ...
                                        $simpantransaksidiagnosa = $this->m_tran_apotek->simpan_item_diagnosa($datatransaksidiagnosa);
                                ...
                                {   
                                    $error = "Tidak Ada Item transaksi dibaris".$i;
                                   break;
                                }
		...
		//$count = $this->m_hasil_transaksi->count_Apotek($searchField,$searchString,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2);
                $count = $this->m_view_hasilapotek->countDiagnosa($id_trans);
		//$count = 1;
		$count > 0 ? $total_pages = ceil($count/$limit) : $total_pages = 0;
		if ($page > $total_pages) $page=$total_pages;
		$start = $limit*$page - $limit;
		...
		//$data1 = $this->m_hasil_transaksi->get_Apotek($searchField,$searchString, $sidx, $sord, $limit, $start,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2)->result();
                $data1 = $this->m_view_hasilapotek->getDiagnosa($id_trans)->result();
        ...
        function detail_item($id_trans)
	{
		...
		//$count = $this->m_hasil_transaksi->count_Apotek($searchField,$searchString,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2);
                $count = $this->m_view_hasilapotek->countItem($id_trans);
		//$count = 1;
		$count > 0 ? $total_pages = ceil($count/$limit) : $total_pages = 0;
		if ($page > $total_pages) $page=$total_pages;
		$start = $limit*$page - $limit;
		...
		//$data1 = $this->m_hasil_transaksi->get_Apotek($searchField,$searchString, $sidx, $sord, $limit, $start,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2)->result();
                $data1 = $this->m_view_hasilapotek->getItem($id_trans)->result_array();
		...
		{
			//$responce->rows[$i]['id_transaksi']   = $line->id_transaksi;
                        $responce->rows[$i]['id_transaksi']   = $row['id_item_transaksi_apotek'];
                        $jumlah += $row['jumlah'];
			...
			$responce->rows[$i]['cell'] = array($row['id_item_transaksi_apotek'],$row['jenis_item'],$row['nama_item'],$row['jumlah'],$row['harga_item'],$row['hrg_satuan'],null,null,$row['nama_dosis'],$row['nama_rekomendasi'],$tot);
			$i++;
		}
        ...
        {
              $data['id_item']=$id;
	      $this->load->view('hasil/edititem',$data); //Default di arahkan ke function grid
        }
