...
class Hasilgigi extends CI_Controller {
   // $kun="";
    function __construct() 
    {
        parent::__construct();
        $this->load->model(array('m_rekomendasi','m_hasil_transaksi','m_transaksi','m_karyawan','m_tran_gigi','m_tertanggung','m_item','m_dokter','m_lab_gigi','m_view_hasilgigi'));
    ...
    function index() 
    {
	$this->load->view('hasil/hasilgigi'); //Default di arahkan ke function grid
    //echo "Semangat";
	}
	...
	{	$jt = $this->input->get('jns_tanggal');
		...
		$count = $this->m_hasil_transaksi->count_Gigi($searchField,$searchString,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2,$jns_tanggal);
		...
		$data1 = $this->m_hasil_transaksi->get_Gigi($searchField,$searchString, $sidx, $sord, $limit, $start,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2,$jns_tanggal)->result();
	...
	// Add some data
        ...
        foreach($data1 as $line)
		{
      	...
      	// Rename worksheet
	$this->excel->getActiveSheet()->setTitle('Gigi');
	...
	header('Content-Disposition: attachment;filename="Transaksi Gigi.xls"');
	...
	$msg = "";
	$pesan="";
	$fileElementName = 'uploadhasilgigi';
	...
	else
	{
			...
			else
			{
				          ...
				          $this->m_karyawan->insert_karyawan($datanya);
                                      ...
                                      if($pasien!=null)
                                      {
                                      $data_pasien = $this->m_tran_gigi->cek_pasien($id_karyawan,$pasien);
				          ...
				          $this->m_tertanggung->insert_tertanggung($datapasien);
                                      ...
                                      if($id_pasien==null)
                                      {
                                        ...
                                        $tran=$this->m_tran_gigi->simpan_transaksi($datatransaksi);
                                        ...
                                        {
                                        $pesan .="Cari Dokter";
                                        $datadokter = $this->m_tran_gigi->caridokterrujukan($nama_dokter,7,3);
                                        if($datadokter)
                                        {
                                        ...
                                        {
                                        $datanya=array('nama_dokter'=>$nama_dokter,'gol_dokter'=>3,'kat_dokter'=>7);
				        $doktere=$this->m_dokter->insert_dokter($datanya);
                                        ...
                                        {
                                        $dataprovider = $this->m_tran_gigi->carilabgigi($nama_provider);
                                        if($dataprovider)
                                        {
				        ...
				        $this->m_lab_gigi->insert_lab_gigi($datagigi);
                                        $dataprovider = $this->m_tran_gigi->carilabgigi($nama_provider);
                                        $id_provider = $dataprovider[0]['id_provider'];
                                        
                                        //$pesan .="tidak ada provider";
                                        }
                                        }
                                        if($id_provider==null)
                                        {
                                        ...
                                        //$pesan .="insert transaksi gigi".$id_transaksi."dok".$id_dokter."gigi".$id_provider."bukti".$no_bukti."resti".$restitusi;
                                        $datatransaksigigi =  array('id_transaksi'=>$id_transaksi,'id_dokter'=>$id_dokter,'id_provider'=>$id_provider,'no_bukti'=>$no_bukti,'restitusi'=>$restitusi,'no_surat'=>$no_surat);
                                        $query3 = $this->m_tran_gigi->simpan_transaksi_gigi($datatransaksigigi);
                                        ...
                                        $query4 = $this->m_tran_gigi->simpan_transaksi_buku($databuku);
                                    ...
                                    $pesan .="Nama item".$nama_item;
                                    $pesan .="Id _tem".$id_item;
                                    if($dosis!=null)
                                    {
                                        $caridosis = $this->m_tran_gigi->caridosis($dosis);
                                        if($caridosis)
                                        {
                                            //$row = $caridosis->row();
                                        ...
                                        {
                                            $query = $this->m_tran_gigi->get_id_dosis();
                                            if( ! empty($query) )
                                            {
                                            $row = $query->row();
                                            $id=$row->id_dosis+1;
                                            ...
                                            $this->m_tran_gigi->simpan_dosis($datadosis);
                                            $id_dosis=$id;
                                            }
                                        }
                                        $pesan .="Item".$id_item."Dosis".$id_dosis."transaksi".$id_transaksi;
                                        //$dataitemdosis = array('id_item'=>$id_item,'id_dosis'=>$id_dosis,'id_transaksi'=>$id_transaksi);
                                        //$query4 = $this->m_tran_gigi->simpan_item_dosis($dataitemdosis);
                            ...
                            if(($id_transaksi != null) && ($diagnosa!=null))
                                        {
                                        $caridiagnosa=$this->m_tran_gigi->caridiagnosa($diagnosa);
                                        if($caridiagnosa)
                                        {
                                        ...
                                        $this->m_tran_gigi->simpan_diagnosa($data_diagnosa);
                                        $caridiagnosa=$this->m_tran_gigi->caridiagnosa($diagnosa);
                                        $id_diagnosa = $caridiagnosa[0]['id_diagnosa'];
                                        ...
                                        $simpantransaksidiagnosa = $this->m_tran_gigi->simpan_item_diagnosa($datatransaksidiagnosa);
                            ...
                            else if (($sheetData[$i]['P'])==null && ($sheetData[$i]['C'])==null)
                            {
                                break;
                            }
                            }
		...
		//$count = $this->m_hasil_transaksi->count_Apotek($searchField,$searchString,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2);
                $count = $this->m_view_hasilgigi->countDiagnosa($id_trans);
		//$count = 1;
		$count > 0 ? $total_pages = ceil($count/$limit) : $total_pages = 0;
		if ($page > $total_pages) $page=$total_pages;
		$start = $limit*$page - $limit;
		...
		//$data1 = $this->m_hasil_transaksi->get_Apotek($searchField,$searchString, $sidx, $sord, $limit, $start,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2)->result();
                $data1 = $this->m_view_hasilgigi->getDiagnosa($id_trans)->result();
        ...
        function detail_item($id_trans)
	{
		...
		//$count = $this->m_hasil_transaksi->count_Apotek($searchField,$searchString,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2);
                $count = $this->m_view_hasilgigi->countItem($id_trans);
		//$count = 1;
		$count > 0 ? $total_pages = ceil($count/$limit) : $total_pages = 0;
		if ($page > $total_pages) $page=$total_pages;
		$start = $limit*$page - $limit;
		...
		//$data1 = $this->m_hasil_transaksi->get_Apotek($searchField,$searchString, $sidx, $sord, $limit, $start,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2)->result();
                $data1 = $this->m_view_hasilgigi->getItem($id_trans)->result_array();
		...
		{
			//$responce->rows[$i]['id_transaksi']   = $line->id_transaksi;
                        $responce->rows[$i]['id_transaksi']   = $row['id_item_transaksi_gigi'];
                        $jumlah += $row['jumlah'];
			...
			$responce->rows[$i]['cell'] = array($row['id_item_transaksi_gigi'],$row['jenis_item'],$row['nama_item'],$row['jumlah'],$row['harga_item'],$row['hrg_satuan'],null,null,$row['nama_dosis'],$row['nama_rekomendasi'],$tot);
			$i++;
		}
...
function simpandiagnosa()
    {
        ...
        //mengecek diagnosa sdh ada apa g' di db
            ...
            if (!empty($id_kun))
            {
            $query2 = $this->m_tran_gigi->simpan_item_diagnosa($data2);
                if($query2){
                    echo "sukses";
                } else {
                    echo "gagal";
     ...
     function cruddiagnosa()
		...
		{
	        case 'del':
	           $this->m_tran_gigi->delete_diagnosa($id);
	        break;
		}
        }
		...
		{
	        case 'del':
	           $this->m_tran_gigi->delete_transaksi($id);
	        break;
		}
        }
