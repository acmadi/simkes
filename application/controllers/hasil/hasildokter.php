...
class Hasildokter extends CI_Controller {
   // $kun="";
    function __construct() 
    {
        parent::__construct();
        $this->load->model(array('m_tran_dokter','m_rekomendasi','m_hasil_transaksi','m_view_hasildokter','m_view_hasilapotek','m_dokter','m_karyawan','m_tran_dokter','m_tertanggung','m_transaksi','m_apotek','m_item'));
    ...
    function index() 
    {
	$this->load->view('hasil/hasildokter'); //Default di arahkan ke function grid
    //echo "Semangat";
	}
	...
	{	$jt = $this->input->get('jns_tanggal');
		...
		$count = $this->m_hasil_transaksi->count_Dokter($searchField,$searchString,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2,$jns_tanggal);
		...
		$data1 = $this->m_hasil_transaksi->get_Dokter($searchField,$searchString, $sidx, $sord, $limit, $start,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2,$jns_tanggal)->result();
		...
		foreach($data1 as $line)
		{
                ...
                else
                {
                    ...
                    $responce->rows[$i]['cell'] = array($line->id_transaksi,$line->tgl_transaksi,$line->tgl_kunjungan,$line->no_surat,$line->no_bukti,$line->buku_besar,$line->restitusi,$line->nip,$line->nama_karyawan,$line->nama_tertanggung,$line->nama_dokter,$diagnosa,$line->total_standar,$line->total_satuan);
	...
	// Add some data
		...
		$data1 = $this->m_hasil_transaksi->get_Dokter($searchField,$searchString, $sidx, $sord, $limit, $start,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2,$jns_tanggal)->result();
                $tulis=$this->excel->setActiveSheetIndex(0);
                        ...
                        $tulis->setCellValue('N1', "TARIF STANDAR");
                        $tulis->setCellValue('O1', 'tarif satuan');
        ...
        foreach($data1 as $line)
		{
			$tulis->setCellValue('A'.$i, $i-1);
                        ...
                        $tulis->setCellValue('N'.$i, $line->tarif_standar);
                        $tulis->setCellValue('O'.$i, 'tarif satuan');
			$i++;
		}
	...
	header('Content-Disposition: attachment;filename="Transaksi Dokter.xls"');
	...
	//exit;
		...
		//$count = $this->m_hasil_transaksi->count_Apotek($searchField,$searchString,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2);
                $count = $this->m_view_hasildokter->countDiagnosa($id_trans);
		//$count = 1;
		$count > 0 ? $total_pages = ceil($count/$limit) : $total_pages = 0;
		if ($page > $total_pages) $page=$total_pages;
		$start = $limit*$page - $limit;
		...
		//$data1 = $this->m_hasil_transaksi->get_Apotek($searchField,$searchString, $sidx, $sord, $limit, $start,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2)->result();
                $data1 = $this->m_view_hasildokter->getDiagnosa($id_trans)->result();
        ...
        function detail_item($id_trans)
	{
		...
		//$count = $this->m_hasil_transaksi->count_Apotek($searchField,$searchString,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2);
                $count = $this->m_view_hasildokter->countItem($id_trans);
		//$count = 1;
		$count > 0 ? $total_pages = ceil($count/$limit) : $total_pages = 0;
		if ($page > $total_pages) $page=$total_pages;
		$start = $limit*$page - $limit;
		...
		//$data1 = $this->m_hasil_transaksi->get_Apotek($searchField,$searchString, $sidx, $sord, $limit, $start,$rayon,$wilayah,$mitra,$bulan,$tahun,$tgl1,$tgl2)->result();
                $data1 = $this->m_view_hasildokter->getItem($id_trans)->result_array();
		...
		{
			//$responce->rows[$i]['id_transaksi']   = $line->id_transaksi;
                        $responce->rows[$i]['id_transaksi']   = $row['id_transaksi_dokter'];
                        $jumlah += $row['jumlah'];
			...
			$responce->rows[$i]['cell'] = array($row['id_transaksi_dokter'],$row['jenis_item'],$row['nama_item'],$row['jumlah'],$row['harga_item'],$row['hrg_satuan'],null,null,$row['nama_dosis'],$row['nama_rekomendasi'],$tot);
			$i++;
		}
	...
	$msg = "";
	$pesan="";
	$fileElementName = 'uploadhasildokter';
	...
	else
	{
			...
			else
			{
                            ...
                            for ($i = 7; $i <=count($sheetData); $i++)
                            {
                                ...
                                if(!$this->validasi->cekLengthString($disetujui,1))
                                {
                                $error="Data Disetujui di baris ke".$i." hanya boleh y atau t";
                                break;
                                }
                                if($jml != null)
                                {
				          ...
				          $this->m_karyawan->insert_karyawan($datanya);
                                      ...
                                      if($pasien!=null)
                                      {
                                      $data_pasien = $this->m_tran_dokter->cek_pasien($id_karyawan,$pasien);
...
//                                      break;
                                      if($id_pasien==null)
                                      {
                                        ...
                                        $pesan .="id_transaksi".$id_transaksi.",id_tertanggung".$id_pasien.",tgl_transaksi".$tgl_trans.",tgl_kunjungan";
                                        $datatransaksi = array('id_transaksi'=>$id_transaksi,'id_tertanggung'=>$id_pasien,'tgl_transaksi'=>$tgl_trans,'tgl_kunjungan'=>$tgl_kunj);
                                        $tran = $this->m_tran_dokter->simpan_transaksi($datatransaksi);
                                      
                                        if($tran)
                                        {
                                        if($rujukan!=null)
                                        {
                                        $datarujukan = $this->m_tran_dokter->carirujukan($rujukan);
                                        if($datarujukan)
                                        {
                                        ...
                                        {
                                        $pesan .="Cari Dokter";
                                        $datadokter = $this->m_tran_dokter->caridokterrujukan($nama_dokter,$id_rujukan,3);
                                        if($datadokter)
                                        {
				        ...
				        $doktere=$this->m_dokter->insert_dokter($datanya);
                                        ...
                                        $dataprovider = $this->m_tran_dokter->cariapotek($nama_provider);
                                        if($dataprovider)
                                        {
				        ...
				        $this->m_apotek->insert_apotek($dataapotek);
                                        ...
                                        $datatransaksidokter =  array('id_transaksi'=>$id_transaksi,'id_dokter'=>$id_dokter,'no_bukti'=>$no_bukti,'restitusi'=>$restitusi);
                                        //$pesan .="data transaksi id_transaksi,id_dokter,no_bukti,restitusi";
                                        $query3 = $this->m_tran_dokter->simpan_transaksi_dokter($datatransaksidokter);
                                        ...
                                        $query4 = $this->m_tran_dokter->simpan_transaksi_buku($databuku);
                                        }
                                        else
                                        {
                                    ...
                                    //$pesan .="Cari sheetdata[".$i."][o]item = ".$nama_item;
                                        ...
                                        $cari_item=$this->m_tran_dokter->caritagihan($nama_item,1);
                                    if( ! empty($cari_item))
                                    {
                                    ...
                                    if($dosis!=null)
                                    {
                                        $caridosis = $this->m_tran_dokter->caridosis($dosis);
                                        if($caridosis)
                                        {
                                            //$row = $caridosis->row();
                                        ...
                                        {
                                            $query = $this->m_tran_dokter->get_id_dosis();
                                            if( ! empty($query) )
                                            {
                                            $row = $query->row();
                                            $id=$row->id_dosis+1;
                                    ...
                                    $dataitemtransaksi=array('id_transaksi'=>$id_transaksi,'disetujui'=>$disetujui,'hrg_satuan'=>$hrg_satuan,'jumlah'=>$jml,'racikan'=>$formularium_item,'id_item'=>$id_item,'total'=>$total,'id_rekomendasi'=>$id_rekomendasi);
                                    //$pesan .="Insert Item Transaksi";
                                    $insertitem=$this->m_tran_dokter->simpan_item_transaksi($dataitemtransaksi);
                                    }
                                ...
                                if(($id_transaksi != null) && ($diagnosa!=null))
                                    {
                                        $caridiagnosa=$this->m_tran_dokter->caridiagnosa($diagnosa);
                                        if($caridiagnosa)
                                        {
                                        ...
                                        $this->m_tran_dokter->simpan_diagnosa($data_diagnosa);
                                        $caridiagnosa=$this->m_tran_dokter->caridiagnosa($diagnosa);
                                        $id_diagnosa = $caridiagnosa[0]['id_diagnosa'];
                                        ...
                                        $simpantransaksidiagnosa = $this->m_tran_dokter->simpan_item_diagnosa($datatransaksidiagnosa);
        ...
        function simpandiagnosa()
    {
        ...
        if($id_diag=='undefined')
        {
            ...
            $this->m_tran_dokter->simpan_diagnosa($data);
            $id_diag=$this->m_tran_dokter->get_id_diagnosa();
        }
        ...
        //mengecek diagnosa sdh ada apa g' di db
            ...
            if (!empty($id_kun))
            {
            $query2 = $this->m_tran_dokter->simpan_item_diagnosa($data2);
                if($query2){
                    echo "sukses";
                } else {
                    echo "gagal";
     ...
     function cruddiagnosa()
		...
		{
	        case 'del':
	           $this->m_tran_dokter->delete_diagnosa($id);
	        break;
		}
        }
